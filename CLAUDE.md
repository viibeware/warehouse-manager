...
**Universal part locations (v1.9.7).** One app-wide warehouse/bin list in the `part_locations` app_settings blob: `[{name, bins: [...]}]`. Parts still store a single free-text `parts.location` string in the canonical `Warehouse · Bin` form (`LOCATION_SEP`); the list only drives suggestions, so typed values are never rejected or rewritten. Helpers `_split_location` / `_join_location` / `_normalize_part_locations` / `_part_location_suggestions` / `_add_part_location` sit next to `_module_enabled`; `migrate_v45` seeds the list from existing `parts.location` values and no-ops if the blob already exists. Routes: `GET /api/settings/part-locations` (any user — the part form needs it), `PUT` (admin, full replace), `POST .../add` (editor, folds in one typed value — backs the form's "+ Add to the list" offer). All three go through `_inventory_guard()`. Frontend: a single global `<datalist id="partLocationOptions">` feeds the form's Location input (native datalist deliberately — `.modal-body` is the scroll container, so a floating dropdown would clip); the editor lives in `partLocationsModal`, reached from Settings → Modules → Inventory → Locations.

**Migration system.** Schema changes are append-only functions `migrate_vN(conn)` registered in the `MIGRATIONS` list. `init_db()` takes an fcntl lock on `.migration_lock` before applying anything — this is required because Gunicorn starts multiple workers that all call `init_db()` on boot. To add a schema change: write `migrate_vN`, append `(N, migrate_vN)` to `MIGRATIONS`, done. Never edit a past migration; add a new one. Current head is v45.
