...
See [CHANGELOG.md](./CHANGELOG.md) for the full release history. The in-app About pane fetches from the same file on each build, so a single edit keeps README, the shipped image, and the UI in sync.
